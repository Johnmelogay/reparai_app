...
import { ProviderCard, SkeletonProviderCard } from '@/components/ui/ProviderCard';
...
// Provider cards extracted to @/components/ui/ProviderCard



// Static User Location Marker (Avoids Reanimated inside Marker which causes iOS Native Crash)
const StaticUserMarker = () => {
    return (
...
export default function HomeScreen() {
    ...
    const { providers: nearbyProviders, loading, error: partnersError, refetch } = usePartners();
    ...
    const filteredProviders = useMemo(() => {
        // Hard cap map markers to keep iOS simulator stable with live updates.
        const MAX_MAP_MARKERS = 80;
        const sorted = [...nearbyProviders].sort((a, b) => {
            const aOnline = String(a?.status || '').toLowerCase() === 'online' ? 1 : 0;
            const bOnline = String(b?.status || '').toLowerCase() === 'online' ? 1 : 0;
            if (aOnline !== bOnline) return bOnline - aOnline;

            const aDist = typeof a.rawDistance === 'number' ? a.rawDistance : Number.MAX_SAFE_INTEGER;
            const bDist = typeof b.rawDistance === 'number' ? b.rawDistance : Number.MAX_SAFE_INTEGER;
            return aDist - bDist;
        });
                ...
                <View style={styles.root}>
                    ...
                    >
                        ...
                        {filteredProviders.map((provider) => {
                            ...
                            const { image: categoryImage } = getCategoryPinMeta(provider?.category);
                            ...
                            return (
                                <Marker
                                    ...
                                    tracksViewChanges={false}
                                >
                                    <View style={styles.homeProviderMarkerWrap}>
                                        <View style={styles.homeProviderAvatar}>
                                            {provider.image && provider.image !== 'https://via.placeholder.com/150' ? (
                                                <Image
                                                    source={{ uri: provider.image }}
                                                    style={styles.homeProviderImageFull}
                                                    resizeMode="cover"
                                                />
                                            ) : (
                                                <View style={styles.homeProviderPlaceholder}>
                                                    <Image
                                                        source={categoryImage}
                                                        style={styles.homeProviderImageSmall}
                                                        resizeMode="contain"
                                                    />
                                                </View>
                                            )}
                                        ...
                                        <View style={[styles.homeStatusDot, { backgroundColor: statusColor }]} />

                                        {/* Rating Pill */}
                                        <View style={styles.homeRatingPill}>
                                            <Star size={8} color="#F59E0B" fill="#F59E0B" />
                                            <Text style={styles.homeRatingText}>{ratingText}</Text>
                                        </View>
                                    </View>
                                </Marker>
                            );
...
const styles = StyleSheet.create({
    ...
    homeProviderImageFull: {
        width: '100%',
        height: '100%',
    },
    homeProviderPlaceholder: {
        width: '100%',
        height: '100%',
