...
export default function SelectProviderScreen() {
    ...
    const { providers, loading: providersLoading, error: providersError, refetch } = usePartners();
    ...
    // Generate offers from real provider data
    useEffect(() => {
        if (providersLoading) return;

        const categoryFilters = resolveCategoryFilters(category);

        // Filter real providers by category
        ...
        const generatedOffers = matchedProviders.map(provider => ({
            provider,
            price: provider.hourlyRate || Math.floor(Math.random() * 100) + 50,
            estimatedTime: Math.floor(Math.random() * 30) + 15,
            message: `Olá! Posso ajudar com ${category || 'seu serviço'}.`,
        }));
    ...
    return (
        <SafeAreaView style={styles.container} edges={['top']}>
            ...
            >
                <NetworkErrorBanner
                    visible={!!providersError}
                    message={providersError || undefined}
                    onRetry={refetch}
                />
