    ...
    const getPlaceDetails = useCallback(async (placeId: string): Promise<PlaceDetails | null> => {
        ...
        try {
            ...
            if (data.addressComponents) {
                const numberComponent = data.addressComponents.find((c: any) => c.types.includes('street_number'));
                if (numberComponent) {
                    extractedStreetNumber = numberComponent.longText || numberComponent.shortText || '';
                }
            }
